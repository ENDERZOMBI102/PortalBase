	...
	for ( i = 0; i < source->numpoints; i++ ) {
		...
		for ( j = 0; j < pass->numpoints; j++ ) {
			...
			for ( k = 0; k < source->numpoints; k++ ) {
				...
				if ( d < -ON_VIS_EPSILON ) {  // source is on the negative side, so we want all
					// pass and target on the positive side
					fliptest = false;
					break;
				} else if ( d > ON_VIS_EPSILON ) {  // source is on the positive side, so we want all
...
void RecursiveLeafFlow( int leafnum, threaddata_t* thread, pstack_t* prevstack ) {
	...
	for ( i = 0; i < leaf->portals.Count(); i++ ) {
		...
		if ( !more && CheckBit( thread->base->portalvis, pnum ) ) {// can't see anything new
			continue;
		}
