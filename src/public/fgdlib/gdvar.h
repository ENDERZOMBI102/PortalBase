...
//-----------------------------------------------------------------------------
struct GDIVITEM {
	uint32 iValue;               // Bitflag value for ivFlags
...
public:
	...
	[[nodiscard]]
	inline const char* GetLongName() const { return m_szLongName; }
	...
	[[nodiscard]]
	inline int GetChoiceCount() const { return m_Items.Count(); }
	...
	inline void GetDefault( int* pnStore ) const {
		pnStore[0] = m_nDefault;
	}

	inline void GetDefault( char* pszStore ) const {
	...
	inline bool IsReportable() const;
	inline bool IsReadOnly() const;

	GDinputvariable& operator=( const GDinputvariable& pOther );
...
// Purpose:
//-----------------------------------------------------------------------------
const char* GDinputvariable::GetDescription() const {
...
//-----------------------------------------------------------------------------
bool GDinputvariable::IsReadOnly() const {
	return m_bReadOnly;
}
...
//			Report dialog.
//-----------------------------------------------------------------------------
bool GDinputvariable::IsReportable() const {
...
int GDinputvariable::GetFlagMask( int nFlag ) {
	...
	return static_cast<int32>( m_Items.Element( nFlag ).iValue );
}
