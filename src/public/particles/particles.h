...
public:
	...
	void AddBuiltinSimulationOperators();
	void AddBuiltinRenderingOperators();
	...
	// create a particle system by name. returns nullptr if one of that name does not exist
	...
	IParticleSystemQuery* Query() { return m_pQuery; }
	...
	void DumpProfileInformation();// write particle_profile.csv
...
public:
	...
	ALWAYS_INLINE float TotalRecordedExecutionTime() const {
		return m_flTotalExecutionTime;
	}

	ALWAYS_INLINE float MaximumRecordedExecutionTime() const {
...
public:
	...
	virtual uint32 GetWrittenAttributes() const = 0;
	virtual uint32 GetReadAttributes() const = 0;
	virtual uint64 GetReadControlPointMask() const {
		return 0;
	}

	//  Used when an operator needs to read the attributes of a particle at spawn time
	virtual uint32 GetReadInitialAttributes() const {
		return 0;
	}
	...
	virtual bool IsFinalConstraint() const {
		return false;
	}
	...
	virtual bool ShouldRunBeforeEmitters() const {
		return false;
	}

	// Does this operator require that particles remain in the order they were emitted?
	virtual bool RequiresOrderInvariance() const {
		return false;
	}
	...
	virtual ~CParticleOperatorInstance() {
...
public:
	...
	void Render( IMatRenderContext* pRenderContext, bool bTranslucentOnly = false, void* pCameraObject = nullptr );
	...
	int GetHighestControlPoint() const;
	...
	bool CheckIfOperatorShouldRun( CParticleOperatorInstance const* op, float* pflCurStrength = nullptr );
...
private:
	...
	bool HasAttachedKillList() const;
...
inline bool CParticleCollection::HasAttachedKillList() const {
...
inline void CParticleCollection::SwapPosAndPrevPos() {
...
inline void CParticleCollection::SetControlPointOrientation( int nWhichPoint, const Vector& forward,
	...
	// check perpendicular
	if ( fabs( DotProduct( forward, up ) ) <= 0.1f && fabs( DotProduct( forward, right ) ) <= 0.1f && fabs( DotProduct( right, up ) ) <= 0.1f ) {
...
// Inline methods
//-----------------------------------------------------------------------------
inline CParticleSystemDefinition::CParticleSystemDefinition() {
...
inline int CParticleCollection::GetGroupID() const {
...
ALWAYS_INLINE bool CParticleCollection::IsValid() const {
