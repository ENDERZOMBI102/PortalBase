...
struct vgui::DragDrop_t {
	DragDrop_t() : m_bDragEnabled( false ),
...
BoundKey_t::BoundKey_t() : isbuiltin( true ),
						   bindingname( 0 ),
						   keycode( KEY_NONE ),
						   modifiers( 0 ) {
}
...
KeyBindingMap_t::KeyBindingMap_t() : bindingname( 0 ),
									 func( 0 ),
									 helpstring( 0 ),
									 docstring( 0 ),
									 passive( false ) {
...
public:
	CKeyBindingsMgr() : m_Bindings( 0, 0, KeyBindingContextHandleLessFunc ),
						m_nKeyBindingContexts( 0 ) {
	}

	struct KBContext_t {
		KBContext_t() : m_KeyBindingsFile( UTL_INVAL_SYMBOL ),
...
Panel* Panel::m_sMousePressedPanels[] = { nullptr, nullptr, nullptr };
...
void Panel::Init( int x, int y, int wide, int tall ) {
	...
	m_NavLeft = nullptr;
	m_NavRight = nullptr;
	m_NavToRelay = nullptr;
	m_NavActivate = nullptr;
	m_NavBack = nullptr;
	m_sNavUpName = nullptr;
	m_sNavDownName = nullptr;
	m_sNavLeftName = nullptr;
	m_sNavRightName = nullptr;
	m_sNavToRelayName = nullptr;
	m_sNavActivateName = nullptr;
	m_sNavBackName = nullptr;
...
Panel::~Panel() {
	...
	while ( ipanel()->GetChildCount( GetVPanel() ) ) {
		...
		if ( ipanel()->IsAutoDeleteSet( child ) ) {
			...
			ipanel()->SetParent( child, NULL_PANEL_HANDLE );
		}
	}
...
Panel* Panel::GetParent() {
	...
	// the root node always returns nullptr when a GetParent() is done so everything is OK
	if ( ipanel() ) {
...
void Panel::OnScreenSizeChanged( int nOldWide, int nOldTall ) {
	...
	for ( int i = 0; i < ipanel()->GetChildCount( GetVPanel() ); i++ ) {
		...
		PostMessage( child, new KeyValues( "OnScreenSizeChanged", "oldwide", nOldWide, "oldtall", nOldTall ), NULL_PANEL_HANDLE );
	}
...
void Panel::PaintTraverse( bool repaint, bool allowForce ) {
	...
	bool bBorderPaintFirst = _border != nullptr && _border->PaintFirst();
...
void Panel::SetParent( VPANEL newParent ) {
	if ( newParent ) {
		...
		ipanel()->SetParent( GetVPanel(), NULL_PANEL_HANDLE );
	}
...
// Output : Panel * - nullptr if no panel of that name is found
//-----------------------------------------------------------------------------
...
void Panel::InternalMousePressed( int code ) {
	...
	if ( IsBuildGroupEnabled() ) {
		...
		if ( ( vgui::input()->IsKeyDown( KEY_LCONTROL ) || vgui::input()->IsKeyDown( KEY_RCONTROL ) ) && ( vgui::input()->IsKeyDown( KEY_LALT ) || vgui::input()->IsKeyDown( KEY_RALT ) ) ) {
	...
	if ( IsBuildGroupEnabled() ) {
		...
		if ( Q_stristr( pGameDir, "tf" ) && tf_strict_mouse_up_events.GetBool() ) {
			...
			if ( code >= MOUSE_LEFT && code <= MOUSE_MIDDLE ) {
				...
				m_sMousePressedPanels[ nIndex ] = nullptr;// Clear out pressed panel
...
void Panel::ReloadKeyBindings() {
	...
	#define KEY_NAME( str, disp ) \
		{ KEY_##str, MAKE_STRING( KEY_##str ), disp }
...
void Panel::InternalSetCursor() {
	...
	if ( visible ) {
			...
			if ( m_pDragDrop->m_bDragging || g_DragDropCapture.Get() != nullptr ) {
				return;
			}
		#endif
...
bool Panel::IsBuildModeActive() {
	return _buildGroup && _buildGroup->IsEnabled();
}
...
void Panel::RequestFocus( int direction ) {
	...
	OnRequestFocus( GetVPanel(), NULL_PANEL_HANDLE );
}
...
// Purpose:
//-----------------------------------------------------------------------------
void Panel::UpdateSiblingPin() {
	if ( !_pinToSibling ) {
		ipanel()->SetSiblingPin( GetVPanel(), NULL_PANEL_HANDLE );
		return;
	}
...
int Panel::ComputeTall( KeyValues* inResourceData, int nParentWide, int nParentTall, bool bComputingOther ) {
	...
	const char* tstr = inResourceData->GetString( "tall", nullptr );
	if ( tstr ) {
...
void Panel::ApplySettings( KeyValues* inResourceData ) {
	...
	_buildModeFlags &= ~( BUILDMODE_SAVE_XPOS_RIGHTALIGNED | BUILDMODE_SAVE_XPOS_CENTERALIGNED | BUILDMODE_SAVE_YPOS_BOTTOMALIGNED | BUILDMODE_SAVE_YPOS_CENTERALIGNED | BUILDMODE_SAVE_WIDE_FULL | BUILDMODE_SAVE_TALL_FULL | BUILDMODE_SAVE_PROPORTIONAL_TO_PARENT | BUILDMODE_SAVE_WIDE_PROPORTIONAL_TALL | BUILDMODE_SAVE_TALL_PROPORTIONAL_WIDE | BUILDMODE_SAVE_XPOS_PROPORTIONAL_PARENT | BUILDMODE_SAVE_YPOS_PROPORTIONAL_PARENT | BUILDMODE_SAVE_XPOS_PROPORTIONAL_SELF | BUILDMODE_SAVE_YPOS_PROPORTIONAL_SELF );
	...
	PinCorner_e pinOurCornerToSibling = GetPinCornerFromString( inResourceData->GetString( "pin_corner_to_sibling", nullptr ) );
	PinCorner_e pinSiblingCorner = GetPinCornerFromString( inResourceData->GetString( "pin_to_sibling_corner", nullptr ) );
	...
	const char* pKeyboardInputEnabled = inResourceData->GetString( "keyboardinputenabled", nullptr );
...
//-----------------------------------------------------------------------------
void Panel::ApplyOverridableColors() {
	for ( int i = 0; i < m_OverridableColorEntries.Count(); i++ ) {
		if ( m_OverridableColorEntries[ i ].m_bOverridden ) {
			( *m_OverridableColorEntries[ i ].m_pColor ) = m_OverridableColorEntries[ i ].m_colFromScript;
		}
...
//-----------------------------------------------------------------------------
const char* Panel::GetDescription() {
...
//			Returns a pointer to a valid panel, nullptr if the panel has been deleted
//-----------------------------------------------------------------------------
Panel* PHandle::Get() {
...
//-----------------------------------------------------------------------------
// Purpose: Returns a handle to a valid panel, nullptr if the panel has been deleted
//-----------------------------------------------------------------------------
...
Panel* Panel::GetDropTarget( CUtlVector<KeyValues*>& msglist ) {
	#if defined( VGUI_USEDRAGDROP )
		// Found one
		if ( m_pDragDrop->m_bDropEnabled && IsDroppable( msglist ) ) {
			return this;
		}
...
namespace vgui {
	...
	SortedPanel_t::SortedPanel_t( Panel* panel ) {
		pPanel = panel;
		pButton = dynamic_cast<Button*>( panel );
	}
	...
	void VguiPanelGetSortedChildButtonList( Panel* pParentPanel, void* pSortedPanels, const char* pchFilter /*= nullptr*/, int nFilterType /*= 0*/ ) {
		CUtlSortVector<SortedPanel_t, CSortedPanelYLess>* pList = reinterpret_cast<CUtlSortVector<SortedPanel_t, CSortedPanelYLess>*>( pSortedPanels );
		...
		for ( int i = 0; i < pParentPanel->GetChildCount(); i++ ) {
			...
			if ( pchFilter && pchFilter[ 0 ] != '\0' ) {
				...
				// Substring
					...
					if ( V_strstr( szBuff, pchFilter ) == nullptr ) {
						continue;
					}
				}
			}
	...
	int VguiPanelNavigateSortedChildButtonList( void* pSortedPanels, int nDir ) {
		auto* pList = reinterpret_cast<CUtlSortVector<SortedPanel_t, CSortedPanelYLess>*>( pSortedPanels );
