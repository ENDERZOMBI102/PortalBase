...
struct PanelItem_t {
	...
	Panel* m_EditLabel{nullptr};
	TextEntry* m_EditPanel{nullptr};
	ComboBox* m_pCombo{nullptr};
...
public:
	...
	void AddItem( Panel* label, TextEntry* edit, ComboBox* combo, Button* button, const char* name, const int type ) {
		PanelItem_t item;
	...
	void RemoveAll() {
		for ( int i = 0; i < m_PanelList.Count(); i += 1 ) {
...
public:
	...
	BuildModeLocalizedStringEditDialog() : Frame( this, nullptr ) {
		...
		for ( int i = 0; i < g_pVGuiLocalize->GetLocalizationFileCount(); i += 1 ) {
	...
	virtual void DoModal( const char* token ) {
		...
		// lookup the value
		const StringIndex_t val = g_pVGuiLocalize->FindIndex( token );
...
void CBuildModeDialogMgr::Add( BuildModeDialog* pDlg ) {
	if ( m_vecBuildDialogs.Find( pDlg ) == CUtlVector<vgui::BuildModeDialog*>::InvalidIndex() ) {
...
public:
	CBuildModeNavCombo( Panel* parent, const char* panelName, int numLines, bool allowEdit, bool getParents, Panel* context )
		: BaseClass( parent, panelName, numLines, allowEdit ), m_bParents( getParents ) {
		m_hContext = context;
	}

	void OnShowMenu( Menu* menu ) override {
		menu->DeleteAllItems();
		if ( not m_hContext.Get() ) {
			return;
		}
		...
		if ( m_bParents ) {
			...
			while ( p ) {
				auto* ep = dynamic_cast<EditablePanel*>( p );
				if ( ep && ep->GetBuildGroup() ) {
					auto kv = new KeyValues( "Panel" );
					kv->SetPtr( "ptr", p );
			...
			for ( int i = 0; i < c; i += 1 ) {
				auto* ep = dynamic_cast<EditablePanel*>( m_hContext->GetChild( i ) );
...
void BuildModeDialog::CreateControls() {
	...
	for ( int i = 0; i < m_pBuildGroup->GetRegisteredControlSettingsFileCount(); i += 1 ) {
	...
	if ( vars && vars->GetFirstSubKey() ) {
		...
		auto menu = new Menu( m_pVarsButton, "VarsMenu" );
...
void BuildModeDialog::ApplySchemeSettings( IScheme* pScheme ) {
	...
	const HFont font = pScheme->GetFont( "DefaultVerySmall" );
...
//-----------------------------------------------------------------------------
void BuildModeDialog::RemoveAllControls() {
	// free the array
...
void BuildModeDialog::SetActiveControl( Panel* controlToEdit ) {
	...
	while ( true ) {
		...
		if ( not stricmp( dataType, "int" ) ) {
			datat = TYPE_STRING;  //!! just for now
		} else if ( not stricmp( dataType, "alignment" ) ) {
			datat = TYPE_ALIGNMENT;
		} else if ( not stricmp( dataType, "autoresize" ) ) {
			datat = TYPE_AUTORESIZE;
		} else if ( not stricmp( dataType, "corner" ) ) {
			datat = TYPE_CORNER;
		} else if ( not stricmp( dataType, "localize" ) ) {
		...
		auto* label = new Label( this, nullptr, fieldName );
		...
		const HFont smallFont = scheme()->GetIScheme( GetScheme() )->GetFont( "DefaultVerySmall" );
...
void BuildModeDialog::OnCommand( const char* command ) {
	...
	} else if ( not stricmp( command, "Exit" ) ) {
		// exit build mode
		ExitBuildMode();
	} else if ( not stricmp( command, "Apply" ) ) {
		// apply data to controls
		ApplyDataToControls();
	} else if ( not stricmp( command, "DeletePanel" ) ) {
		OnDeletePanel();
	} else if ( not stricmp( command, "RevertToSaved" ) ) {
		RevertToSaved();
...
void BuildModeDialog::ApplyDataToControls() {
	...
	const Panel* panel = m_pBuildGroup->FieldNameTaken( fieldName );
	if ( panel ) {
		if ( panel != m_pCurrentPanel )  // make sure name is taken by some other panel not this one
		{
			...
			auto* errorBox = new MessageBox( "Cannot Apply", messageString );
			errorBox->DoModal();
			UpdateControlData( m_pCurrentPanel );
			m_pApplyButton->SetEnabled( false );
			return;
	...
	auto dat = new KeyValues( m_pCurrentPanel->GetName() );
...
KeyValues* BuildModeDialog::StoreSettings() {
	auto storedSettings = new KeyValues( m_pCurrentPanel->GetName() );
...
void BuildModeDialog::OnTextChanged( Panel* panel ) {
	...
	if ( panel == m_pEditableChildren ) {
		...
		if ( kv ) {
			auto* ep = reinterpret_cast<EditablePanel*>( kv->GetPtr( "ptr" ) );
			if ( ep ) {
...
// Purpose:
//-----------------------------------------------------------------------------
void BuildModeDialog::ExitBuildMode() {
...
void BuildModeDialog::ShowHelp() {
	...
	auto* helpDlg = new MessageBox( "Build Mode Help", helpText, this );
	...
	if ( !current || current == context ) {
		...
		if ( i < children ) {
			for ( int offset = 1; offset < children; ++offset ) {
				...
				auto bm = dynamic_cast<BuildModeDialog*>( check );
				if ( bm ) {
					continue;
				}
