...
char* _V_strstr( const char* file, int line, const char* s1, const char* search ) {
	...
	// FIXME: Does this need to not be const?
	return const_cast<char*>( strstr( s1, search ) );
}
...
char* V_strupr( char* start ) {
	auto* str = reinterpret_cast<unsigned char*>( start );
	while ( *str ) {
		if ( static_cast<unsigned char>( *str - 'a' ) <= ( 'z' - 'a' ) )
			*str -= 'a' - 'A';
		else if ( static_cast<unsigned char>( *str ) >= 0x80 )  // non-ascii, fall back to CRT
			*str = toupper( *str );
		str++;
	}
	return start;
...
int V_stricmp( const char* str1, const char* str2 ) {
	...
	for ( ; *s1; ++s1, ++s2 ) {
		if ( *s1 != *s2 ) {
			...
			if ( c1 != c2 || static_cast<unsigned char>( c1 - 'a' ) > ( 'z' - 'a' ) ) {
				// if non-ascii mismatch, fall back to CRT for locale
				if ( ( c1 | c2 ) >= 0x80 )
					return stricmp( reinterpret_cast<const char*>( s1 ), reinterpret_cast<const char*>( s2 ) );
				// ascii mismatch. only use the | 0x20 value if alphabetic.
				if ( static_cast<unsigned char>( c1 - 'a' ) > ( 'z' - 'a' ) )
					c1 = *s1;
				if ( static_cast<unsigned char>( c2 - 'a' ) > ( 'z' - 'a' ) )
					c2 = *s2;
				return c1 > c2 ? 1 : -1;
			}
		}
	...
	for ( ; n > 0 && *s1; --n, ++s1, ++s2 ) {
		if ( *s1 != *s2 ) {
			...
			if ( c1 != c2 || static_cast<unsigned char>( c1 - 'a' ) > ( 'z' - 'a' ) ) {
				// if non-ascii mismatch, fall back to CRT for locale
				if ( ( c1 | c2 ) >= 0x80 )
					return strnicmp( reinterpret_cast<const char*>( s1 ), reinterpret_cast<const char*>( s2 ), n );
				// ascii mismatch. only use the | 0x20 value if alphabetic.
				if ( static_cast<unsigned char>( c1 - 'a' ) > ( 'z' - 'a' ) )
					c1 = *s1;
				if ( static_cast<unsigned char>( c2 - 'a' ) > ( 'z' - 'a' ) )
...
int V_strncmp( const char* s1, const char* s2, int count ) {
	...
	while ( count > 0 ) {
		if ( *s1 != *s2 )
			return static_cast<unsigned char>( *s1 ) < static_cast<unsigned char>( *s2 ) ? -1 : 1;  // string different
		if ( *s1 == '\0' )
...
int V_atoi( const char* str ) {
	return static_cast<int>( V_atoi64( str ) );
}
...
const char* V_stristr( const char* pStr, const char* pSearch ) {
	...
	while ( *pLetter != 0 ) {
		...
		if ( FastToLower( static_cast<unsigned char>( *pLetter ) ) == FastToLower( static_cast<unsigned char>( *pSearch ) ) ) {
			// Check for match
			...
			while ( *pTest != 0 ) {
				...
				if ( FastToLower( static_cast<unsigned char>( *pMatch ) ) != FastToLower( static_cast<unsigned char>( *pTest ) ) )
					break;
...
char* V_stristr( char* pStr, const char* pSearch ) {
	...
	// TODO: Are these casts actually necessary?
	return const_cast<char*>( V_stristr( const_cast<const char*>( pStr ), pSearch ) );
}
...
char* V_strncat( char* pDest, const char* pSrc, size_t destBufferSize, int max_chars_to_copy ) {
	...
	Assert( static_cast<ptrdiff_t>( destBufferSize ) >= 0 );
	...
	if ( max_chars_to_copy <= COPY_ALL_CHARACTERS ) {
		...
		charstocopy = static_cast<size_t>( std::min( max_chars_to_copy, static_cast<int>( srclen ) ) );
	}
	...
	if ( static_cast<int>( charstocopy ) <= 0 ) {
		return pDest;
	}
...
wchar_t* V_wcsncat( INOUT_Z_CAP( cchDest ) wchar_t* pDest, const wchar_t* pSrc, size_t cchDest, int max_chars_to_copy ) {
	...
	Assert( static_cast<ptrdiff_t>( cchDest ) >= 0 );
...
char* V_pretifymem( float value, int digitsafterdecimal /*= 2*/, bool usebinaryonek /*= false*/ ) {
	...
	if ( FloatMakePositive( value - static_cast<int>( value ) ) < 0.00001 ) {
...
bool Q_RemoveAllEvilCharacters( char* pch ) {
	...
	auto* pwch = static_cast<wchar_t*>( stackalloc( cubDest ) );
...
bool Q_StripPrecedingAndTrailingWhitespaceW( wchar_t* pwch ) {
	...
	// duplicate on stack
	int cubDest = ( cch + 1 ) * sizeof( wchar_t );
	auto* pwchT = static_cast<wchar_t*>( stackalloc( cubDest ) );
...
int _V_UCS2ToUnicode( const ucs2* pUCS2, wchar_t* pUnicode, int cubDestSizeInBytes ) {
	...
	pUnicode[ 0 ] = 0;
		...
		char* pIn = const_cast<char*>( reinterpret_cast<const char*>( pUCS2 ) );
		char* pOut = reinterpret_cast<char*>( pUnicode );
		if ( conv_t != nullptr ) {
...
int _V_UnicodeToUCS2( const wchar_t* pUnicode, int cubSrcInBytes, char* pUCS2, int cubDestSizeInBytes ) {
		...
		auto* pDest = reinterpret_cast<wchar_t*>( pUCS2 );
		...
		char* pIn = const_cast<char*>( reinterpret_cast<const char*>( pUnicode ) );
		char* pOut = pUCS2;
		if ( conv_t != nullptr ) {
			cchResult = iconv( conv_t, &pIn, &nLenUnicde, &pOut, &nMaxUCS2 );
			iconv_close( conv_t );
			if ( static_cast<int>( cchResult ) < 0 )
				cchResult = 0;
			else
				cchResult = cubSrcInBytes / sizeof( wchar_t );
		}
	#else
		#error "_V_UnicodeToUCS2: Must be implemented for this platform"
	#endif
	return cchResult;
}
...
int _V_UCS2ToUTF8( const ucs2* pUCS2, char* pUTF8, int cubDestSizeInBytes ) {
	...
	pUTF8[ 0 ] = 0;
		...
		char* pIn = const_cast<char*>( reinterpret_cast<const char*>( pUCS2 ) );
		char* pOut = const_cast<char*>( pUTF8 );
		if ( conv_t != nullptr ) {
...
char* AllocString( const char* pStr, int nMaxChars ) {
	...
	if ( nMaxChars == -1 )
		...
		allocLen = std::min( static_cast<int>( strlen( pStr ) ), nMaxChars ) + 1;
...
int iHexCharToInt( char cValue ) {
	auto iValue = static_cast<int32>( static_cast<uint8>( cValue ) );
	if ( static_cast<uint32>( iValue - '0' ) < 10 )
		return iValue - '0';

	iValue |= 0x20;
	if ( static_cast<uint32>( iValue - 'a' ) < 6 )
		return iValue - 'a' + 10;
...
size_t Q_URLDecodeInternal( char* pchDecodeDest, int nDecodeDestLen, const char* pchEncodedSource, int nEncodedSourceLen, bool bUsePlusForSpace ) {
	...
	return static_cast<size_t>( iDestPos );
}

//-----------------------------------------------------------------------------
// Purpose: Encodes a string (or binary data) to URL encoding format, see rfc1738 section 2.2.
...
bool BGetLocalFormattedDateAndTime( time_t timeVal, char* pchDate, int cubDate, char* pchTime, int cubTime ) {
	...
	if ( timeVal ) {
		...
		struct tm tmStruct{};
		struct tm tmToDisplay = *( Plat_localtime( const_cast<const time_t*>( &timeVal ), &tmStruct ) );
		#if IsPosix()
			if ( pchDate != nullptr ) {
				pchDate[ 0 ] = 0;
...
bool BGetLocalFormattedDate( time_t timeVal, char* pchDate, int cubDate ) {
	return BGetLocalFormattedDateAndTime( timeVal, pchDate, cubDate, nullptr, 0 );
}
