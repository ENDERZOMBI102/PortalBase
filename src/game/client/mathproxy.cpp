...
void CAddProxy::OnBind( void* pC_BaseEntity ) {
	...
	switch ( resultType ) {
		...
		case MATERIAL_VAR_TYPE_INT:
			m_pResult->SetFloatValue( static_cast<float32>( m_pSrc1->GetIntValue() ) + static_cast<float32>( m_pSrc2->GetIntValue() ) );
			break;
	...
	switch ( resultType ) {
		...
		case MATERIAL_VAR_TYPE_INT:
			if ( m_pSrc2->GetIntValue() != 0 ) {
				...
				m_pResult->SetFloatValue( static_cast<float32>( m_pSrc1->GetIntValue() ) );
			}
			break;
		default:
			break;
...
void CSineProxy::OnBind( void* pC_BaseEntity ) {
	...
	// get a value in [0,1]
	float32 flValue = std::sinf( 2.0f * static_cast<float32>( M_PI ) * ( gpGlobals->curtime - flSineTimeOffset ) / flSinePeriod ) * 0.5f + 0.5f;
...
void CFracProxy::OnBind( void* pC_BaseEntity ) {
	...
	switch ( resultType ) {
		case MATERIAL_VAR_TYPE_VECTOR: {
			Vector a;
			m_pSrc1->GetVecValue( a.Base(), vecSize );
			a[0] -= static_cast<float32>( static_cast<int32>( a[0] ) );
			a[1] -= static_cast<float32>( static_cast<int32>( a[1] ) );
			a[2] -= static_cast<float32>( static_cast<int32>( a[2] ) );
			m_pResult->SetVecValue( a.Base(), vecSize );
			break;
		}
		...
		case MATERIAL_VAR_TYPE_FLOAT: {
			...
			a -= static_cast<float32>( static_cast<int32>( a ) );
			SetFloatResult( a );
			break;
		}
...
void CLinearRampProxy::OnBind( void* pC_BaseEntity ) {
	...
	// get a value in [0,1]
	SetFloatResult( m_Rate.GetFloat() * gpGlobals->curtime + m_InitialValue.GetFloat() );
...
void CUniformNoiseProxy::OnBind( void* pC_BaseEntity ) {
	SetFloatResult( RandomFloat( m_flMinVal.GetFloat(), m_flMaxVal.GetFloat() ) );
...
void CGaussianNoiseProxy::OnBind( void* pC_BaseEntity ) {
	...
	float flVal = RandomFloat( flMean, flStdDev );
...
public:
	...
	IMaterial* GetMaterial() override { return nullptr; }
};
...
void CLessOrEqualProxy::OnBind( void* pC_BaseEntity ) {
	...
	switch ( resultType ) {
		...
		case MATERIAL_VAR_TYPE_INT:
			m_pResult->SetFloatValue( static_cast<float32>( pSourceVar->GetIntValue() ) );
			break;
...
void CWrapMinMaxProxy::OnBind( void* pC_BaseEntity ) {
	...
	if ( m_flMaxVal.GetFloat() <= m_flMinVal.GetFloat() )  // Bad input, just return the min
	{
		...
		if ( flResult >= 0.0f ) {
			flResult -= static_cast<float32>( static_cast<int32>( flResult ) );
		} else  // Negative
		{
			flResult -= static_cast<float32>( static_cast<int32>( flResult ) - 1 );
		}
	...
	switch ( resultType ) {
		...
		case MATERIAL_VAR_TYPE_INT:
			if ( m_pSrc1->GetIntValue() ) {
				...
				m_pResult->SetFloatValue( static_cast<float32>( m_pSrc2->GetIntValue() ) );
			}
			break;
