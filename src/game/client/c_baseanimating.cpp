...
bool C_AnimationLayer::IsActive() {
...
void C_ClientRagdoll::OnSave() {
}

void C_ClientRagdoll::OnRestore() {
	...
	if ( pRagdollT == nullptr || pRagdollT->list[ 0 ].pObject == nullptr ) {
	...
	// this must be recomputed because the model was nullptr when this was set up
...
void C_ClientRagdoll::HandleAnimatedFriction() {
	if ( m_iFrictionAnimState == RAGDOLL_FRICTION_OFF )
		return;

	ragdoll_t* pRagdollT = nullptr;
	int iBoneCount = 0;
...
void C_ClientRagdoll::SUB_Remove() {
	m_bFadingOut = true;
	SetNextClientThink( CLIENT_THINK_ALWAYS );
}

void C_ClientRagdoll::ClientThink() {
...
//-----------------------------------------------------------------------------
C_BaseAnimating::C_BaseAnimating() : m_iv_flCycle( "C_BaseAnimating::m_iv_flCycle" ),
...
bool C_BaseAnimating::UsesPowerOfTwoFrameBufferTexture() {
...
void C_BaseAnimating::LockStudioHdr() {
	Assert( m_hStudioHdr == MDLHANDLE_INVALID && m_pStudioHdr == nullptr );

	AUTO_LOCK( m_StudioHdrInitLock );

	if ( m_hStudioHdr != MDLHANDLE_INVALID || m_pStudioHdr != nullptr ) {
...
void C_BaseAnimating::GetBoneTransform( int iBone, matrix3x4_t& pBoneToWorld ) {
	...
	CBoneCache* pcache = GetBoneCache( nullptr );
...
//-----------------------------------------------------------------------------
void C_BaseAnimating::InitModelEffects() {
...
void C_BaseAnimating::CreateUnragdollInfo( C_BaseAnimating* pRagdoll ) {
	...
	pRagdoll->SetupBones( nullptr, -1, BONE_USED_BY_ANYTHING, gpGlobals->curtime );
...
CollideType_t C_BaseAnimating::GetCollideType() {
	if ( IsRagdoll() )
	...
	for ( C_BaseEntity* pChild = FirstMoveChild(); pChild; pChild = pChild->NextMovePeer() ) {
		...
		if ( pChildAnimating ) {
			...
			if ( pChildHdr && pChild->IsEffectActive( EF_BONEMERGE ) && pChildHdr->SequencesAvailable() && pChildAnimating->m_pBoneMergeCache ) {
				...
				childBoneSetup.AccumulatePose( childPos, childQ, 0, GetCycle(), 1.0, currentTime, nullptr );
...
void C_BaseAnimating::StandardBlendingRules( CStudioHdr* hdr, Vector pos[], Quaternion q[], float currentTime, int boneMask ) {
	...
	UnragdollBlend( hdr, pos, q, currentTime );
		...
		#if IsDebug()
			if ( Q_stristr( hdr->pszName(), r_sequence_debug.GetString() ) != nullptr ) {
...
bool C_BaseAnimating::CalcAttachments() {
	...
	return SetupBones( nullptr, -1, BONE_USED_BY_ATTACHMENT, gpGlobals->curtime );
}
...
// Purpose:
//-----------------------------------------------------------------------------
void C_BaseAnimating::UpdateOnRemove() {
...
CBoneCache* C_BaseAnimating::GetBoneCache( CStudioHdr* pStudioHdr ) {
	...
	SetupBones( nullptr, -1, boneMask, gpGlobals->curtime );
	...
	for ( int i = 0; i < targetCount; i++ ) {
		...
		switch ( pTarget->type ) {
			case IK_GROUND: {
				...
				if ( trace.m_pEnt != nullptr && trace.m_pEnt->GetMoveType() == MOVETYPE_PUSH ) {
...
CMouthInfo* C_BaseAnimating::GetMouth() {
	return &m_mouth;
}

#if defined( DEBUG_BONE_SETUP_THREADING )
...
bool C_BaseAnimating::SetupBones( matrix3x4_t* pBoneToWorldOut, int nMaxBones, int boneMask, float currentTime ) {
	...
	//=============================================================================
	// HPE_BEGIN:
	// [pfreese] Added the check for pBoneToWorldOut != nullptr in this debug warning
	...
	if ( pBoneToWorldOut != nullptr && !IsBoneAccessAllowed() ) {
	...
	if ( m_iMostRecentModelBoneCounter != g_iModelBoneCounter ) {
		...
		#if defined( STUDIO_ENABLE_PERF_COUNTERS )
			CStudioHdr* hdr = GetModelPtr();
			if ( hdr ) {
...
int C_BaseAnimating::DrawModel( int flags ) {
	...
	if ( r_drawothermodels.GetInt() ) {
		...
		if ( ( flags & ( STUDIO_SSAODEPTHTEXTURE | STUDIO_SHADOWDEPTHTEXTURE ) ) == 0 &&
			 g_pStudioStatsEntity != nullptr && g_pStudioStatsEntity == GetClientRenderable() ) {
	...
	if ( vcollide_wireframe.GetBool() ) {
		if ( IsRagdoll() ) {
			...
			if ( pCollide && pCollide->solidCount == 1 ) {
				...
				engine->DebugDrawPhysCollide( pCollide->solids[ 0 ], nullptr, matrix, debugColor );
...
int C_BaseAnimating::InternalDrawModel( int flags ) {
	...
	matrix3x4_t* pBoneToWorld = nullptr;
	bool bMarkAsDrawn = modelrender->DrawModelSetup( *pInfo, &state, nullptr, &pBoneToWorld );
	...
	DoInternalDrawModel( pInfo, ( bMarkAsDrawn && ( pInfo->flags & STUDIO_RENDER ) ) ? &state : nullptr, pBoneToWorld );
...
//			full_reset -
//-----------------------------------------------------------------------------
void C_BaseAnimating::ResetLatched() {
	// Reset the IK
	if ( m_pIk ) {
		delete m_pIk;
		m_pIk = nullptr;
...
// implements these so ragdolls can handle frustum culling & leaf visibility
//-----------------------------------------------------------------------------
const Vector& C_BaseAnimating::GetRenderOrigin() {
	if ( IsRagdoll() ) {
...
const QAngle& C_BaseAnimating::GetRenderAngles() {
	if ( IsRagdoll() ) {
		return vec3_angle;
...
void C_BaseAnimating::RagdollMoved() {
...
// Purpose:
//-----------------------------------------------------------------------------
void C_BaseAnimating::AddEntity() {
...
float C_BaseAnimating::GetAnimTimeInterval() const {
	#define MAX_ANIMTIME_INTERVAL 0.2f

	float flInterval = std::min( gpGlobals->curtime - m_flAnimTime, MAX_ANIMTIME_INTERVAL );
	return flInterval;
}
...
void C_BaseAnimating::ResetSequenceInfo() {
	if ( GetSequence() == -1 ) {
...
// Output : int
//-----------------------------------------------------------------------------
int C_BaseAnimating::GetHitboxSet() {
	return m_nHitboxSet;
}
...
// Output : char const
//-----------------------------------------------------------------------------
const char* C_BaseAnimating::GetHitboxSetName() {
	if ( IsDynamicModelLoading() )
		return "";
...
// Output : int
//-----------------------------------------------------------------------------
int C_BaseAnimating::GetHitboxSetCount() {
	if ( IsDynamicModelLoading() )
		return 0;
...
void C_BaseAnimating::ClearRagdoll() {
	if ( m_pRagdoll ) {
		// immediately mark the member ragdoll as being nullptr,
...
void C_BaseAnimating::GetAimEntOrigin( IClientEntity* pAttachedTo, Vector* pAbsOrigin, QAngle* pAbsAngles ) {
	...
	if ( IsEffectActive( EF_BONEMERGE ) && IsEffectActive( EF_BONEMERGE_FASTCULL ) && ( pMoveParent = GetMoveParent() ) != nullptr ) {
...
void C_BaseAnimating::RefreshCollisionBounds() {
...
//-----------------------------------------------------------------------------
bool C_BoneFollower::ShouldDraw() {
...
int C_BoneFollower::DrawModel( int flags ) {
	...
	if ( pCollide ) {
		...
		matrix3x4_t matrix;
		AngleMatrix( GetAbsAngles(), GetAbsOrigin(), matrix );
		engine->DebugDrawPhysCollide( pCollide->solids[ m_solidIndex ], nullptr, matrix, debugColor );
	}
	return 1;
}
...
LocalFlexController_t C_BaseAnimating::GetNumFlexControllers() {
	CStudioHdr* pstudiohdr = GetModelPtr();
	if ( !pstudiohdr )
...
//-----------------------------------------------------------------------------
unsigned char C_BaseAnimating::GetClientSideFade() {
...
//-----------------------------------------------------------------------------
void C_BaseAnimating::SetReceivedSequence() {
...
//-----------------------------------------------------------------------------
bool C_BaseAnimating::ShouldResetSequenceOnNewModel() {
...
// Purpose:
//-----------------------------------------------------------------------------
void C_BaseAnimating::UpdateBoneAttachments() {
	if ( !m_pAttachedTo )
		return;
