...
#if defined( INTERPOLATEDVAR_PARANOID_MEASUREMENT )
...
C_BasePlayer* C_BaseEntity::m_pPredictionPlayer = nullptr;
...
public:
	...
	virtual int GetPredictableCount();
...
// Output : int
//-----------------------------------------------------------------------------
int CPredictableList::GetPredictableCount() {
...
// Functions.
//-----------------------------------------------------------------------------
C_BaseEntity::C_BaseEntity() : m_iv_vecOrigin( "C_BaseEntity::m_iv_vecOrigin" ),
...
void C_BaseEntity::Clear() {
	...
	m_ShadowDirUseOtherEntity = nullptr;
...
// Purpose:
//-----------------------------------------------------------------------------
C_BaseEntity* C_BaseEntity::GetShadowUseOtherEntity() const {
...
// Output : Index
//-----------------------------------------------------------------------------
int C_BaseEntity::entindex() const {
	return index;
}
...
// Output : const
//-----------------------------------------------------------------------------
const QAngle& C_BaseEntity::GetAbsAngles() const {
	//Assert( s_bAbsQueriesValid );
	const_cast<C_BaseEntity*>( this )->CalcAbsolutePosition();
...
bool C_BaseEntity::IgnoresZBuffer() const {
...
bool C_BaseEntity::GetSoundSpatialization( SpatializationInfo_t& info ) {
	...
	// pModel might be nullptr, but modelinfo can handle that
...
// Output : float * - The clip plane to use, or nullptr if no clip plane is defined
//-----------------------------------------------------------------------------
...
void C_BaseEntity::UnlinkChild( C_BaseEntity* pParent, C_BaseEntity* pChild ) {
	...
	// Unlink from parent
	// NOTE: pParent *may well be nullptr*! This occurs
	...
	pChild->m_pMovePeer = nullptr;
	pChild->m_pMovePrevPeer = nullptr;
	pChild->m_pMoveParent = nullptr;
...
#define MAX_INTERPOLATE_VELOCITY_PLAYER 1250.0f
	...
	//-----------------------------------------------------------------------------
	void C_BaseEntity::CreateLightEffects() {
		dlight_t* dl;
...
#include "tier0/memdbgon.h"
	...
	// Output : int
	//-----------------------------------------------------------------------------
	int C_BaseEntity::GetTeamNumber() const {
		return m_iTeamNum;
	}
...
*/
	...
	const Vector& C_BaseEntity::GetLocalOrigin() const {
		return m_vecOrigin;
	}
	...
	// Prevent these for now until hierarchy is properly networked
	const QAngle& C_BaseEntity::GetLocalAngles() const {
...
*/
	...
	//-----------------------------------------------------------------------------
	void C_BaseEntity::InitPredictable() {
#if !defined( NO_ENTITY_PREDICTION )
		Assert( !GetPredictable() );
	...
	// Output : Returns true on success, false on failure.
	//-----------------------------------------------------------------------------
	bool C_BaseEntity::GetPredictable() const {
	...
	// Output : Returns true on success, false on failure.
	//-----------------------------------------------------------------------------
	bool C_BaseEntity::GetPredictionEligible() const {
#if !defined( NO_ENTITY_PREDICTION )
	...
	// Output : Returns true on success, false on failure.
	//-----------------------------------------------------------------------------
	bool C_BaseEntity::IsClientCreated() const {
#ifndef NO_ENTITY_PREDICTION
		if ( m_pPredictionContext != nullptr ) {
	...
	// Output : string_t
	//-----------------------------------------------------------------------------
	string_t C_BaseEntity::GetModelName() const {
		return m_ModelName;
	}
...
// Not hooked up to prediction yet
	...
	// Output : Returns true on success, false on failure.
	//-----------------------------------------------------------------------------
	bool C_BaseEntity::IsDormantPredictable() const {
	...
	// Output : Returns true on success, false on failure.
	//-----------------------------------------------------------------------------
	bool C_BaseEntity::BecameDormantThisPacket() const {
#if !defined( NO_ENTITY_PREDICTION )
	...
	// Output : Returns true on success, false on failure.
	//-----------------------------------------------------------------------------
	bool C_BaseEntity::IsIntermediateDataAllocated() const {
#if !defined( NO_ENTITY_PREDICTION )
		return m_pOriginalData != nullptr ? true : false;
#else
	return false;
#endif
	}
	...
	void C_BaseEntity::AddVar( void* data, IInterpolatedVar* watcher, int type, bool bSetup ) {
		...
		if ( bAddIt ) {
			...
			Assert( watcher->GetDebugName() != nullptr );
	...
	void CC_CL_Find_Ent( const CCommand& args ) {
		...
		C_BaseEntity* ent = nullptr;
		while ( ( ent = ClientEntityList().NextBaseEntity( ent ) ) != nullptr ) {
