...
CON_COMMAND( hud_reloadscheme, "Reloads hud layout and animation scripts." ) {
	auto* mode = static_cast<ClientModeShared*>( GetClientModeNormal() );
	if ( !mode ) {
		return;
	}
...
static void __MsgFunc_Rumble( bf_read& msg ) {
	const unsigned char waveformIndex = msg.ReadByte();
	const unsigned char rumbleData = msg.ReadByte();
	const unsigned char rumbleFlags = msg.ReadByte();
...
static void __MsgFunc_VGUIMenu( bf_read& msg ) {
	...
	const bool bShow = msg.ReadByte() != 0;
	...
	auto* mode = dynamic_cast<ClientModeShared*>( GetClientModeNormal() );
...
ClientModeShared::ClientModeShared() {
	...
	#if defined( REPLAY_ENABLED )
		m_pReplayReminderPanel = nullptr;
...
void ClientModeShared::Init() {
	m_pChatElement = static_cast<CBaseHudChat*>( GET_HUDELEMENT( CHudChat ) );
	Assert( m_pChatElement );

	m_pWeaponSelection = reinterpret_cast<CBaseHudWeaponSelection*>( GET_HUDELEMENT( CHudWeaponSelection ) );
	...
	Assert( m_pViewport );
	m_pViewport->LoadControlSettings( "scripts/HudLayout.res", nullptr, nullptr, pConditions );
...
//-----------------------------------------------------------------------------
bool ClientModeShared::ShouldDrawCrosshair() {
	return true;
}
...
//-----------------------------------------------------------------------------
bool ClientModeShared::ShouldDrawFog() {
	return true;
}
...
void ClientModeShared::Update() {
	...
	if ( cl_show_num_particle_systems.GetBool() ) {
		...
		for ( int i = 0; i < g_pParticleSystemMgr->GetParticleSystemCount(); i++ ) {
			...
			for ( CParticleCollection* pCurCollection = pParticleSystem->FirstCollection(); pCurCollection != nullptr; pCurCollection = pCurCollection->GetNextCollectionUsingSameDef() ) {
				++nCount;
			}
		}
...
int ClientModeShared::KeyInput( int down, ButtonCode_t keynum, const char* pszCurrentBinding ) {
	...
	// if in-game spectator mode, let spectator input intercept key event here
	if ( pPlayer && pPlayer->GetObserverMode() > OBS_MODE_DEATHCAM && !HandleSpectatorKeyInput( down, keynum, pszCurrentBinding ) ) {
		return 0;
	}
...
int ClientModeShared::HandleSpectatorKeyInput( int down, ButtonCode_t keynum, const char* pszCurrentBinding ) {
	...
	if ( down && pszCurrentBinding && Q_strcmp( pszCurrentBinding, "+attack2" ) == 0 ) {
	...
	if ( down && pszCurrentBinding && Q_strcmp( pszCurrentBinding, "+strafe" ) == 0 ) {
...
// Purpose:
//-----------------------------------------------------------------------------
void ClientModeShared::LevelShutdown() {
		...
		if ( !IsInCommentaryMode() ) {
			...
			if ( g_PR ) {
				...
				if ( pszLocString ) {
					...
					auto itemName{ CEconItemLocalizedFullNameGenerator( GLocalizationProvider(), pItemDefinition, iItemQuality ).GetFullName() };
					g_pVGuiLocalize->ConstructString( wszLocalizedString, sizeof( wszLocalizedString ), LOCCHAR( "%s1" ), 1, itemName );
			...
			// Make sure the instance count is reset to 0.  Sometimes the count stays in sync, and we get replay messages displaying lower than they should.
