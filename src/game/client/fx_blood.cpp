...
void FX_BloodSpray( const Vector &origin, const Vector &normal, float scale, unsigned char r, unsigned char g, unsigned char b, int flags )
{
	...
	if (flags & FX_BLOODSPRAY_DROPS)
	{
		...
		for ( i = 0; i < 14; i++ )
		{
			...
			offset = origin;
			offset += right * RandomFloat( -0.5f, 0.5f ) * scale;
			offset += up * RandomFloat( -0.5f, 0.5f ) * scale;
			...
			tParticle->m_vecVelocity = offDir * RandomFloat( 4.0f * scale, 40.0f * scale );
			tParticle->m_vecVelocity[2] += RandomFloat( 4.0f, 16.0f ) * scale;

			tParticle->m_flWidth		= RandomFloat( 0.125f, 0.275f ) * scale;
			tParticle->m_flLength		= RandomFloat( 0.02f, 0.03f ) * scale;
		...
		for ( i = 0; i < 24; i++ )
		{
			...
			offDir[2] += RandomFloat(0, 1.0f);

			tParticle->m_vecVelocity = offDir * RandomFloat( 2.0f * scale, 25.0f * scale );
			tParticle->m_vecVelocity[2] += RandomFloat( 4.0f, 16.0f ) * scale;

			tParticle->m_flWidth		= RandomFloat( 0.25f, 0.375f ) * scale;
			tParticle->m_flLength		= RandomFloat( 0.0025f, 0.005f ) * scale;
			tParticle->m_flDieTime		= RandomFloat( 0.5f, 1.0f );
	...
	if ((flags & FX_BLOODSPRAY_GORE) || (flags & FX_BLOODSPRAY_CLOUD))
	{
		...
		if (flags & FX_BLOODSPRAY_GORE)
		{
			...
			for ( i = 0; i < 6; i++ )
			{
				...
				if ( pParticle != NULL )
				{
					...
					pParticle->m_vecVelocity += normal * RandomInt( 10, 100 );
					...
					pParticle->m_uchStartSize	= RandomFloat( scale * 0.25, scale );
					pParticle->m_uchEndSize		= pParticle->m_uchStartSize * 2;
					
					pParticle->m_uchStartAlpha	= RandomInt( 200, 255 );
		...
		if (flags & FX_BLOODSPRAY_CLOUD)
		{
			...
			for ( i = 0; i < 6; i++ )
			{
				...
				offset += right * RandomFloat( -1, 1 ) * scale;
				offset += up * RandomFloat( -1, 1 ) * scale;
				...
				{
					pParticle->m_flLifetime = 0.0f;
					pParticle->m_flDieTime	= RandomFloat( 0.5f, 0.8f);

					spread = 0.5f;
					pParticle->m_vecVelocity.Random( -spread, spread );
					pParticle->m_vecVelocity += normal * RandomInt( 100, 200 );
					...
					pParticle->m_uchStartSize	= RandomFloat( scale * 1.5f, scale * 2.0f );
					pParticle->m_uchEndSize		= pParticle->m_uchStartSize * 4;
					
					pParticle->m_uchStartAlpha	= RandomInt( 80, 128 );
					pParticle->m_uchEndAlpha	= 0;
...
void FX_BloodBulletImpact( const Vector &origin, const Vector &normal, float scale /*NOTE: Unused!*/, unsigned char r, unsigned char g, unsigned char b )
{
	...
	if ( pParticle != NULL )
	{
		...
		pParticle->m_vecVelocity[2] -= RandomFloat( 8.0f, 16.0f );

		colorRamp = RandomFloat( 0.75f, 2.0f );
	...
	for ( int i = 0; i < 4; i++ )
	{
		...
		{
			pParticle->m_flLifetime = 0.0f;
			pParticle->m_flDieTime	= RandomFloat( 0.5f, 0.75f);

			pParticle->m_vecVelocity	= dir * RandomFloat( 16.0f, 32.0f )*(i+1);
			pParticle->m_vecVelocity[2] -= RandomFloat( 32.0f, 64.0f )*(i+1);
	...
	for ( int i = 0; i < 8; i++ )
	{
		...
		tParticle->m_vecVelocity = offDir * RandomFloat( 64.0f, 128.0f );

		tParticle->m_flWidth		= RandomFloat( 0.5f, 2.0f );
		tParticle->m_flLength		= RandomFloat( 0.05f, 0.15f );
...
void HunterDamageCallback( const CEffectData &data )
{
	...
	for ( int i = 0; i < numFlecks; i++ )
	{
		...
		pFleckParticle = (Particle3D *) pGlassEmitter->AddParticle( sizeof(Particle3D), g_Mat_Fleck_Antlion[RandomInt(0,1)], spawnOffset );

		if ( pFleckParticle == NULL )
			break;

		pFleckParticle->m_flLifeRemaining	= RandomFloat( 2.0f, 3.0f );

		dir[0] = data.m_vNormal[0] + RandomFloat( -flAngularSpray, flAngularSpray );
		dir[1] = data.m_vNormal[1] + RandomFloat( -flAngularSpray, flAngularSpray );
		dir[2] = data.m_vNormal[2] + RandomFloat( -flAngularSpray, flAngularSpray );

		pFleckParticle->m_uchSize		= RandomInt( 3, 8 );

		pFleckParticle->m_vecVelocity	= dir * RandomFloat( flMinSpeed, flMaxSpeed);
