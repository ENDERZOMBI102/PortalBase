...
bool CSpriteTrail::IsInSkybox() const {
	...
	// Compute position	+ bounding box
	//-----------------------------------------------------------------------------
	void CSpriteTrail::UpdateBoundingBox() {
	...
	int CSpriteTrail::DrawModel( int flags ) {
		...
		for ( int i = 0; i <= m_nStepCount; ++i ) {
			...
			BeamSeg_t curSeg;
			curSeg.m_vColor.x = static_cast<float>( m_clrRender->r() ) / 255.0f;
			curSeg.m_vColor.y = static_cast<float>( m_clrRender->g() ) / 255.0f;
			curSeg.m_vColor.z = static_cast<float>( m_clrRender->b() ) / 255.0f;
	...
	// Output : Vector const&
	//-----------------------------------------------------------------------------
	const Vector& CSpriteTrail::GetRenderOrigin() {
	...
	const QAngle& CSpriteTrail::GetRenderAngles() {
		return vec3_angle;
	}
#endif
