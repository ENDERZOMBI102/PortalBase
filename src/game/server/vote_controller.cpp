...
CVoteController* g_voteController = nullptr;
...
public:
	...
	void AddPlayerToKickWatchList( CSteamID steamID, float flDuration ) {
		...
		if ( m_mapKickWatchList.Find( steamID ) == CUtlMap<CSteamID, float>::InvalidIndex() ) {
	...
	void AddPlayerToNameLockedList( CSteamID steamID, float flDuration ) {
		...
		flDuration = clamp( flDuration, 1.f, (float) k_nKickWatchListMaxDuration );
		if ( m_mapNameLockedList.Find( steamID ) == CUtlMap<CSteamID, float>::InvalidIndex() ) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CVoteController::ResetData() {
	...
	for ( int& voteIndex : m_nVotesCast ) {
		voteIndex = VOTE_UNCAST;
	}
}
...
// Purpose:
//-----------------------------------------------------------------------------
int CVoteController::UpdateTransmitState() {
...
// Purpose:
//-----------------------------------------------------------------------------
bool CVoteController::IsVoteSystemEnabled() {
	#ifdef TF_DLL
...
bool CVoteController::CreateVote( int iEntIndex, const char* pszTypeString, const char* pszDetailString ) {
	...
	bool bDedicatedServer = ( iEntIndex == DEDICATED_SERVER );
	...
	for ( int issueIndex = 0; issueIndex < m_potentialIssues.Count(); ++issueIndex ) {
		...
		if ( FStrEq( pszTypeString, pCurrentIssue->GetTypeString() ) ) {
			...
			if ( pCurrentIssue->CanCallVote( iEntIndex, pszDetailString, nErrorCode, nTime ) ) {
				...
				m_acceptingVotesTimer.Start( sv_vote_timer_duration.GetFloat() + RandomFloat( -1.f, 1.f ) );
...
// Purpose:
//-----------------------------------------------------------------------------
void CVoteController::VoteControllerThink() {
	...
	if ( m_acceptingVotesTimer.HasStarted() && m_acceptingVotesTimer.IsElapsed() ) {
		...
		if ( static_cast<float>( GetNumVotesCast() ) >= ( static_cast<float>( m_nPotentialVotes ) * m_potentialIssues[ m_iActiveIssueIndex ]->GetQuorumRatio() ) ) {
		...
		if ( bVotePassed ) {
			// Always nullptr check, as some votes don't target players (i.e. ChangeLevel)
	...
	if ( m_VoteCallers.Count() >= MAX_VOTER_HISTORY ) {
		...
		for ( int iIdx = m_VoteCallers.FirstInorder(); iIdx != CUtlMap<uint64, float>::InvalidIndex(); iIdx = m_VoteCallers.NextInorder( iIdx ) ) {
			if ( m_VoteCallers.Element( iIdx ) - gpGlobals->curtime <= 0 ) {
...
//-----------------------------------------------------------------------------
void CVoteController::CheckForEarlyVoteClose() {
	int nVoteTally = 0;
...
bool CVoteController::IsValidVoter( CBasePlayer* pWhom ) {
	...
	#if !IsDebug() // Don't want to do this check for debug builds (so we can test with bots)
		if ( pWhom->IsBot() )
			return false;
...
//-----------------------------------------------------------------------------
int CVoteController::GetVoteIssueIndexWithHighestCount() {
	int nMaxIndex = -1;
...
void CVoteController::TrackVoteCaller( CBasePlayer* pPlayer ) {
	...
	if ( iIdx != CUtlMap<uint64, float>::InvalidIndex() ) {
		// Already being tracked - update timer
		m_VoteCallers.Element( iIdx ) = gpGlobals->curtime + static_cast<float>( sv_vote_creation_timer.GetInt() );
		return;
	}

	m_VoteCallers.Insert( steamID.ConvertToUint64(), gpGlobals->curtime + static_cast<float>( sv_vote_creation_timer.GetInt() ) );
}
...
// Purpose:
//-----------------------------------------------------------------------------
int CVoteController::GetNumVotesCast() {
	int nVoteTally = 0;
...
// Purpose:
//-----------------------------------------------------------------------------
const char* CBaseIssue::GetTypeString() {
...
// Purpose:
//-----------------------------------------------------------------------------
const char* CBaseIssue::GetDetailsString() {
...
// Purpose:
//-----------------------------------------------------------------------------
bool CBaseIssue::IsTeamRestrictedVote() {
	return false;
}
...
// Purpose:
//-----------------------------------------------------------------------------
const char* CBaseIssue::GetVotePassedString() {
	...
	if ( BRecordVoteFailureEventForEntity( iEntityHoldingVote ) ) {
		...
		for ( int index = 0; index < m_FailedVotes.Count(); index++ ) {
			...
			if ( Q_strcmp( pFailedVote->szFailedVoteParameter, GetDetailsString() ) == 0 ) {
				...
				pFailedVote->flLockoutTime = gpGlobals->curtime + static_cast<float>( nTime );
		...
		auto* pNewFailedVote = new FailedVote;
...
bool CBaseIssue::CanCallVote( int iEntIndex, const char* pszDetails, vote_create_failed_t& nFailCode, int& nTime ) {
	...
	if ( m_flNextCallTime != -1.f && gpGlobals->curtime < m_flNextCallTime ) {
		...
		nTime = static_cast<int>( m_flNextCallTime - gpGlobals->curtime );
		return false;
	}
	...
	for ( int iIndex = 0; iIndex < m_FailedVotes.Count(); iIndex++ ) {
		...
		int nTimeRemaining = static_cast<int>( pCurrentFailure->flLockoutTime - gpGlobals->curtime );
		bool bFailed = false;
...
// Purpose:
//-----------------------------------------------------------------------------
int CBaseIssue::CountPotentialVoters() {
	int nTotalPlayers = 0;
...
// Purpose:
//-----------------------------------------------------------------------------
int CBaseIssue::GetNumberVoteOptions() {
...
// Purpose:
//-----------------------------------------------------------------------------
float CBaseIssue::GetQuorumRatio() {
