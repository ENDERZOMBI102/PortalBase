...
// Output : AI_Response
//-----------------------------------------------------------------------------
bool CAI_Expresser::SpeakFindResponse( AI_Response &outResponse, AIConcept_t _concept, const char *modifiers /*= NULL*/ )
{
	...
	set.AppendCriteria( "concept", _concept, CONCEPT_WEIGHT );
...
// Input  : *response - 
//-----------------------------------------------------------------------------
bool CAI_Expresser::SpeakDispatchResponse( AIConcept_t _concept, AI_Response& response, IRecipientFilter *filter /* = NULL */ )
{
	bool spoke = false;
	...
	if ( IsSpeaking() && _concept[0] != 0 )
	{
		DevMsg( "SpeakDispatchResponse:  Entity ( %i/%s ) already speaking, forcing '%s'\n", GetOuter()->entindex(), STRING( GetOuter()->GetEntityName() ), _concept );
	...
	if ( spoke )
	{
		...
		if ( DebuggingSpeech() && g_pDeveloper->GetInt() > 0 && response.GetType() != RESPONSE_PRINT )
		{
			Vector vPrintPos;
			GetOuter()->CollisionProp()->NormalizedToWorldSpace( Vector(0.5,0.5,1.0f), &vPrintPos );
			NDebugOverlay::Text( vPrintPos, CFmtStr( "%s: %s", _concept, szResponse ), true, 1.5 );
		}
		...
		SetSpokeConcept( _concept, &response );
	}
...
//-----------------------------------------------------------------------------
bool CAI_Expresser::Speak( AIConcept_t _concept, const char *modifiers /*= NULL*/, char *pszOutResponseChosen /* = NULL*/, size_t bufsize /* = 0 */, IRecipientFilter *filter /* = NULL */ )
{
    AI_Response response;
	bool result = SpeakFindResponse( response, _concept, modifiers );
	if ( !result )
		return false;

	SpeechMsg( GetOuter(), "%s (%p) spoke %s (%f)\n", STRING(GetOuter()->GetEntityName()), GetOuter(), _concept, gpGlobals->curtime );

	bool spoke = SpeakDispatchResponse( _concept, response, filter );
...
//-------------------------------------
bool CAI_Expresser::CanSpeakConcept( AIConcept_t _concept )
{
	// Not in history?
	int iter = m_ConceptHistories.Find( _concept );
...
bool CAI_Expresser::SpokeConcept( AIConcept_t _concept )
{
	return GetTimeSpokeConcept( _concept ) != -1.f;
}

//-------------------------------------

float CAI_Expresser::GetTimeSpokeConcept( AIConcept_t _concept )
{
	int iter = m_ConceptHistories.Find( _concept );
	if ( iter == m_ConceptHistories.InvalidIndex() ) 
		return -1;
...
void CAI_Expresser::SetSpokeConcept( AIConcept_t _concept, AI_Response *response, bool bCallback )
{
	int idx = m_ConceptHistories.Find( _concept );
	if ( idx == m_ConceptHistories.InvalidIndex() )
	{
		ConceptHistory_t h;
		h.timeSpoken = gpGlobals->curtime;
		idx = m_ConceptHistories.Insert( _concept, h );
	}
	...
	if ( bCallback )
		GetSink()->OnSpokeConcept( _concept, response );
}

//-------------------------------------

void CAI_Expresser::ClearSpokeConcept( AIConcept_t _concept )
{
	m_ConceptHistories.Remove( _concept );
}
