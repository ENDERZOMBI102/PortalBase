...
AI_Waypoint_t* CAI_Pathfinder::FindShortRandomPath(int startID, float minPathLength, const Vector &directionIn) 
{
			...
			(neighborID != NO_NODE && pAInode[neighborID]->GetType() == NODE_CLIMB))
	{
		...
		if (vDirection != vec3_origin)
		{
			...
			if (numNeighbors == 0)
			{
				neighborID = pStaleNeighbor[RandomInt(0,numStaleNeighbors-1)];
			}
			else
			{
		...
		{
			neighborID = pNeighbor[RandomInt(0,numNeighbors-1)];
		}
		// If no neighbors, try using a stale one
		else
		{
...
public:
		...
		m_pRoute( NULL )
	{
		...
		static_assert( bits_BUILD_GROUND == (int)bits_CAP_MOVE_GROUND && bits_BUILD_FLY == (int)bits_CAP_MOVE_FLY && bits_BUILD_JUMP == (int)bits_CAP_MOVE_JUMP && bits_BUILD_CLIMB == (int)bits_CAP_MOVE_CLIMB );
	}
