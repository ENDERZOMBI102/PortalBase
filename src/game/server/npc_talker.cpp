...
void CNPCSimpleTalker::HandleAnimEvent( animevent_t *pEvent )
{
	...
	case SCRIPT_EVENT_SENTENCE_RND1:		// Play a named sentence group 25% of the time
		if (RandomInt(0,99) < 75)
			break;
		// fall through...
	case SCRIPT_EVENT_SENTENCE:				// Play a named sentence group
		ShutUpFriends();
		PlaySentence( pEvent->options, RandomFloat(2.8, 3.4) );
...
void CNPCSimpleTalker::IdleRespond( void )
{
	...
	DeferAllIdleSpeech( RandomFloat( TALKER_DEFER_IDLE_SPEAK_MIN, TALKER_DEFER_IDLE_SPEAK_MAX ) );
}
...
void CNPCSimpleTalker::SayHelloToPlayer( CBaseEntity *pPlayer )
{
	...
	Speak( TLK_HELLO );
	DeferAllIdleSpeech( RandomFloat( 5, 10 ) );
...
{ 
	...
	if (pFriend && !(pFriend->IsMoving()) && RandomInt( 0, 3 ) != 0 )
	{
		if ( SpeakQuestionFriend( pFriend ) )
		{
			...
			DeferAllIdleSpeech( RandomFloat( TALKER_DEFER_IDLE_SPEAK_MIN, TALKER_DEFER_IDLE_SPEAK_MAX ), pTalkNPC );
			return true;
		}
	}
	*/
...
int CNPCSimpleTalker::PlayScriptedSentence( const char *pszSentence, float delay, float volume, soundlevel_t soundlevel, bool bConcurrent, CBaseEntity *pListener )
{
	...
	DeferAllIdleSpeech( delay + RandomInt( 3.0f, 5.0f ) );
...
int CNPCSimpleTalker::SelectNonCombatSpeechSchedule()
{
	...
	if ( AI_IsSinglePlayer() && GetExpresser()->CanSpeak() && HasCondition ( COND_SEE_PLAYER ) && RandomInt( 0, 6 ) == 0 )
	{
		CBasePlayer *pPlayer = UTIL_GetLocalPlayer();
		Assert( pPlayer );
