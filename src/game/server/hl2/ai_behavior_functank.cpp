...
// Purpose:
//-----------------------------------------------------------------------------
void CAI_FuncTankBehavior::Dismount() {
...
int CAI_FuncTankBehavior::OnTakeDamage_Alive( const CTakeDamageInfo& info ) {
	...
	if ( m_hFuncTank && bValidDismountAttacker ) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CAI_FuncTankBehavior::UpdateOnRemove() {
	if ( m_hFuncTank ) {
		Dismount();
	}
...
// Purpose:
//-----------------------------------------------------------------------------
void CAI_FuncTankBehavior::AimGun() {
...
// Purpose:
//-----------------------------------------------------------------------------
CBaseEntity* CAI_FuncTankBehavior::BestEnemy() {
	...
	for ( AI_EnemyInfo_t* pEMemory = GetEnemies()->GetFirst( &iter ); pEMemory != nullptr; pEMemory = GetEnemies()->GetNext( &iter ) ) {
