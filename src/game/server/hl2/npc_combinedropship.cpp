...
public:
	void Precache();
	void Spawn() override;
	bool OverridePropdata() override;
...
public:
	...
	Class_T Classify() { return CLASS_COMBINE_GUNSHIP; }
...
protected:
	...
	virtual void PopulatePoseParameters();
};
...
//-----------------------------------------------------------------------------
void CNPC_CombineDropship::PopulatePoseParameters() {
...
//-----------------------------------------------------------------------------
bool CCombineDropshipContainer::OverridePropdata() {
	return true;
}
...
// Character killed (only fired once)
//-----------------------------------------------------------------------------
void CCombineDropshipContainer::ThrowFlamingGib() {
	Vector vecAbsMins, vecAbsMaxs;
	CollisionProp()->WorldSpaceAABB( &vecAbsMins, &vecAbsMaxs );
...
void CCombineDropshipContainer::AddSmokeTrail( const Vector& vecPos ) {
	...
	ExplosionCreate( vecPos, vec3_angle, this, 1000, 500.0f, SF_ENVEXPLOSION_NODAMAGE | SF_ENVEXPLOSION_NOSPARKS | SF_ENVEXPLOSION_NODLIGHTS | SF_ENVEXPLOSION_NOSMOKE, 0 );
...
// Output  :
//------------------------------------------------------------------------------
void CNPC_CombineDropship::Spawn() {
	Precache();
	...
	switch ( m_iCrateType ) {
		...
		case CRATE_SOLDIER:
			...
			if ( m_hContainer ) {
				...
				m_hContainer->SetGroundEntity( nullptr );
		...
		case CRATE_APC: {
			m_soldiersToDrop = 0;
			m_hContainer = (CBaseAnimating*) gEntList.FindEntityByName( nullptr, m_iszAPCVehicleName );
			if ( !m_hContainer ) {
			...
			m_hContainer->Teleport( &apcPosition, &apcAngles, nullptr );
	...
	m_hPickupTarget = nullptr;
	m_hLandTarget = nullptr;
...
//-----------------------------------------------------------------------------
void CNPC_CombineDropship::Activate() {
	BaseClass::Activate();
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
void CNPC_CombineDropship::Precache() {
	// Models
			...
			for ( int i = 0; i < DROPSHIP_MAX_SOLDIERS; i++ ) {
				if ( m_sNPCTemplate[ i ] != NULL_STRING ) {
					...
					if ( m_sNPCTemplateData[ i ] != NULL_STRING ) {
						...
						MapEntity_ParseEntity( pEntity, STRING( m_sNPCTemplateData[ i ] ), nullptr );
						if ( pEntity != nullptr ) {
							pEntity->Precache();
...
// Output  :
//------------------------------------------------------------------------------
void CNPC_CombineDropship::Flight() {
	...
	if ( m_hContainer != nullptr && ( m_iCrateType == CRATE_SOLDIER || m_iCrateType == CRATE_NONE ) ) {
...
// Updates the facing direction
//------------------------------------------------------------------------------
void CNPC_CombineDropship::UpdateFacingDirection() {
	if ( GetEnemy() ) {
		...
		if ( GetDesiredPosition().DistToSqr( GetAbsOrigin() ) > 1 ) {
			...
			GetVectors( &m_vecDesiredFaceDir, nullptr, nullptr );
		}
	}
...
// Output  :
//------------------------------------------------------------------------------
void CNPC_CombineDropship::InitializeRotorSound() {
...
void CNPC_CombineDropship::LandCommon( bool bHover ) {
	...
	if ( m_iszLandTarget != NULL_STRING ) {
		CBaseEntity* pTarget = gEntList.FindEntityByName( nullptr, m_iszLandTarget );
		if ( !pTarget ) {
...
void CNPC_CombineDropship::InputDropStrider( inputdata_t& inputdata ) {
	...
	m_hContainer->SetParent( nullptr, 0 );
	m_hContainer->SetOwnerEntity( nullptr );
	m_hContainer->SetAbsAngles( angles );
...
void CNPC_CombineDropship::InputPickup( inputdata_t& inputdata ) {
	...
	CBaseEntity* pTarget = gEntList.FindEntityByName( nullptr, iszTargetName );
	if ( !pTarget ) {
...
// Output  :
//------------------------------------------------------------------------------
void CNPC_CombineDropship::PrescheduleThink() {
	...
	switch ( GetLandingState() ) {
		...
		case LANDING_HOVER_DESCEND: {
			...
			*/
			...
			if ( IsHovering() && m_hLandTarget != nullptr ) {
			...
			if ( !m_flTimeTakeOff ) {
				...
				if ( gpGlobals->curtime > m_flTimeTakeOff ) {
					...
					if ( m_hContainer && m_leaveCrate ) {
						m_hContainer->SetParent( nullptr );
						m_hContainer->SetMoveType( (MoveType_t) m_iContainerMoveType );
...
void CNPC_CombineDropship::DoRotorWash() {
	Vector vecForward;
...
//------------------------------------------------------------------------------
void CNPC_CombineDropship::SpawnTroop() {
	if ( !m_hContainer ) {
		// We're done, take off.
		m_flTimeTakeOff = gpGlobals->curtime + 0.5;
		return;
	...
	vecDeployEndPoint = GetDropoffFinishPosition( vecDeployEndPoint, nullptr, vecNPCMins, vecNPCMaxs );
	...
	CBaseEntity* pEntity = nullptr;
	MapEntity_ParseEntity( pEntity, STRING( m_sNPCTemplateData[ m_iCurrentTroopExiting ] ), nullptr );
...
void CNPC_CombineDropship::InputNPCFinishDustoff( inputdata_t& inputdata ) {
	CBaseEntity* pEnt = gEntList.FindEntityByName( nullptr, inputdata.value.StringID(), nullptr, inputdata.pActivator, inputdata.pCaller );
	if ( !pEnt )
		return;
	...
	CBaseEntity* pDustoff = nullptr;
	if ( m_sDustoffPoints[ m_iCurrentTroopExiting - 1 ] != NULL_STRING ) {
		pDustoff = gEntList.FindEntityByName( nullptr, m_sDustoffPoints[ m_iCurrentTroopExiting - 1 ] );
		if ( !pDustoff ) {
...
// Output  :
//------------------------------------------------------------------------------
float CNPC_CombineDropship::GetAltitude() {
	trace_t tr;
	Vector vecBottom = GetAbsOrigin();
...
//-----------------------------------------------------------------------------
void CNPC_CombineDropship::DropMine() {
...
//------------------------------------------------------------------------------
void CNPC_CombineDropship::UpdatePickupNavigation() {
...
//------------------------------------------------------------------------------
void CNPC_CombineDropship::UpdateLandTargetNavigation() {
...
// Output  :
//------------------------------------------------------------------------------
void CNPC_CombineDropship::Hunt() {
...
//-----------------------------------------------------------------------------
void CNPC_CombineDropship::DoCombatStuff() {
	// Handle mines
	if ( m_bDropMines ) {
		switch ( m_iDropState ) {
			case DROP_IDLE: {
...
//------------------------------------------------------------------------------
bool CNPC_CombineDropship::FireCannonRound() {
...
// Output  :
//------------------------------------------------------------------------------
void CNPC_CombineDropship::StartCannon() {
	m_bIsFiring = true;
...
// Output  :
//------------------------------------------------------------------------------
void CNPC_CombineDropship::StopCannon() {
	m_bIsFiring = false;
