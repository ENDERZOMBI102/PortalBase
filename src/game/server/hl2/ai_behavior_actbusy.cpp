...
public:
	CActBusyAnimData() : CAutoGameSystem( "CActBusyAnimData" ) {
	}
...
// Inherited from IAutoServerSystem
//-----------------------------------------------------------------------------
void CActBusyAnimData::LevelInitPostEntity() {
	ParseAnimDataFile();
}
...
// Purpose:
//-----------------------------------------------------------------------------
void CActBusyAnimData::LevelShutdownPostEntity() {
...
//-----------------------------------------------------------------------------
void CActBusyAnimData::ParseAnimDataFile() {
...
bool CActBusyAnimData::ParseActBusyFromKV( busyanim_t* pAnim, KeyValues* pSection ) {
	...
	// Sequences
	pAnim->iszSequences[ BA_BUSY ] = AllocPooledString( pSection->GetString( "busy_sequence", nullptr ) );
	pAnim->iszSequences[ BA_ENTRY ] = AllocPooledString( pSection->GetString( "entry_sequence", nullptr ) );
	pAnim->iszSequences[ BA_EXIT ] = AllocPooledString( pSection->GetString( "exit_sequence", nullptr ) );

	// Sounds
	pAnim->iszSounds[ BA_BUSY ] = AllocPooledString( pSection->GetString( "busy_sound", nullptr ) );
	pAnim->iszSounds[ BA_ENTRY ] = AllocPooledString( pSection->GetString( "entry_sound", nullptr ) );
	pAnim->iszSounds[ BA_EXIT ] = AllocPooledString( pSection->GetString( "exit_sound", nullptr ) );
...
void CAI_ActBusyBehavior::Enable( CAI_ActBusyGoal* pGoal, float flRange, bool bVisibleOnly ) {
	...
	m_bInQueue = dynamic_cast<CAI_ActBusyQueueGoal*>( m_hActBusyGoal.Get() ) != nullptr;
	m_ForcedActivity = ACT_INVALID;
	m_hSeeEntity = nullptr;
...
void CAI_ActBusyBehavior::OnRestore() {
	if ( m_bEnabled && m_hActBusyGoal != nullptr && IsCombatActBusy() ) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CAI_ActBusyBehavior::Disable() {
	if ( ai_debug_actbusy.GetInt() == 4 ) {
...
void CAI_ActBusyBehavior::ForceActBusyLeave( bool bVisibleOnly ) {
	...
	Enable( nullptr, m_flBusySearchRange, bVisibleOnly );
	m_bForceActBusy = true;
	m_bLeaving = true;
	m_ForcedActivity = ACT_INVALID;
	m_hSeeEntity = nullptr;
...
//-----------------------------------------------------------------------------
void CAI_ActBusyBehavior::StopBusying() {
	if ( !GetOuter() )
		return;
...
// Purpose:
//-----------------------------------------------------------------------------
bool CAI_ActBusyBehavior::CanSelectSchedule() {
...
//-----------------------------------------------------------------------------
bool CAI_ActBusyBehavior::IsCurScheduleOverridable() {
	if ( IsCombatActBusy() ) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CAI_ActBusyBehavior::GatherConditions() {
...
// Purpose:
//-----------------------------------------------------------------------------
void CAI_ActBusyBehavior::EndScheduleSelection() {
	NotifyBusyEnding();
...
//-----------------------------------------------------------------------------
void CAI_ActBusyBehavior::CheckAndCleanupOnExit() {
...
// Purpose:
//-----------------------------------------------------------------------------
void CAI_ActBusyBehavior::BuildScheduleTestBits() {
...
// Purpose:
//-----------------------------------------------------------------------------
int CAI_ActBusyBehavior::SelectScheduleForLeaving() {
			...
			if ( UTIL_DistApprox( GetHintNode()->GetAbsOrigin(), GetAbsOrigin() ) < 64 ) {
				if ( !GetOuter()->IsMarkedForDeletion() ) {
					...
					if ( pOwner ) {
						...
						GetOuter()->SetOwnerEntity( nullptr );
					}
			...
			if ( HasAnimForActBusy( iBusyAnim, BA_BUSY ) ) {
				...
				if ( GetHintNode() && GetHintNode()->Lock( GetOuter() ) ) {
					...
					if ( m_hActBusyGoal && m_hActBusyGoal->m_iszSeeEntityName != NULL_STRING ) {
						...
						m_hSeeEntity.Set( gEntList.FindEntityByName( nullptr, m_hActBusyGoal->m_iszSeeEntityName ) );
					}
...
// Purpose:
//-----------------------------------------------------------------------------
int CAI_ActBusyBehavior::SelectScheduleWhileBusy() {
...
// Purpose:
//-----------------------------------------------------------------------------
bool CAI_ActBusyBehavior::ActBusyNodeStillActive() {
	if ( !GetHintNode() )
		return false;
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CAI_ActBusyBehavior::IsInterruptable() {
	if ( IsActive() )
		return false;
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CAI_ActBusyBehavior::CanFlinch() {
	if ( m_bNeedsToPlayExitAnim )
		return false;
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CAI_ActBusyBehavior::ShouldPlayerAvoid() {
	if ( IsCombatActBusy() ) {
...
//-----------------------------------------------------------------------------
bool CAI_ActBusyBehavior::IsActive() {
...
void CAI_ActBusyBehavior::CollectSafeZoneVolumes( CAI_ActBusyGoal* pActBusyGoal ) {
	...
	if ( pActBusyGoal->m_iszSafeZoneVolume != NULL_STRING ) {
		CBaseEntity* pVolume = gEntList.FindEntityByName( nullptr, pActBusyGoal->m_iszSafeZoneVolume );
...
void CAI_ActBusyBehavior::StartTask( const Task_t* pTask ) {
	switch ( pTask->iTask ) {
		...
		case TASK_ACTBUSY_TELEPORT_TO_BUSY: {
			...
			GetOuter()->Teleport( &vecAbsOrigin, &vecAbsAngles, nullptr );
...
void CAI_ActBusyBehavior::RunTask( const Task_t* pTask ) {
	switch ( pTask->iTask ) {
		...
		case TASK_ACTBUSY_PLAY_BUSY_ANIM: {
			...
			if ( IsCombatActBusy() ) {
				if ( GetEnemy() != nullptr && !HasCondition( COND_ENEMY_OCCLUDED ) ) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CAI_ActBusyBehavior::NotifyBusyEnding() {
...
DEFINE_KEYFIELD( m_flBusySearchRange, FIELD_FLOAT, "busysearchrange" ),
	...
	auto* pActor = dynamic_cast<CAI_BaseNPC*>( pEntity );
	if ( !pActor ) {
...
CAI_ActBusyBehavior* CAI_ActBusyGoal::GetBusyBehaviorForNPC( const char* pszActorName, CBaseEntity* pActivator, CBaseEntity* pCaller, const char* sInputName ) {
	CBaseEntity* pEntity = gEntList.FindEntityByName( nullptr, MAKE_STRING( pszActorName ), nullptr, pActivator, pCaller );
	if ( !pEntity ) {
...
void CAI_ActBusyGoal::InputForceNPCToActBusy( inputdata_t& inputdata ) {
	...
	if ( pszParam ) {
		...
		CBaseEntity* pEntity = gEntList.FindEntityByName( nullptr, pszParam, nullptr, inputdata.pActivator, inputdata.pCaller );
		if ( pEntity ) {
	...
	while ( pszParam ) {
		...
		if ( !Q_strncmp( pszParam, "teleport", 8 ) ) {
			...
			pSeeEntity = gEntList.FindEntityByName( nullptr, pszParam + 4 );
		} else if ( pszParam[ 0 ] == '$' ) {
...
#define QUEUE_MOVEUP_THINK_CONTEXT "ActBusyQueueMoveUpThinkContext"
	...
	// Purpose:
	//-----------------------------------------------------------------------------
	void CAI_ActBusyQueueGoal::Spawn() {
	BaseClass::Spawn();
...
// Purpose:
//-----------------------------------------------------------------------------
void CAI_ActBusyQueueGoal::DrawDebugGeometryOverlays() {
...
void CAI_ActBusyQueueGoal::InputActivate( inputdata_t& inputdata ) {
	if ( !IsActive() ) {
		...
		for ( int i = 0; i < MAX_QUEUE_NODES; i++ ) {
			...
			CBaseEntity* pEntity = gEntList.FindEntityByName( nullptr, m_iszNodes[ i ] );
			if ( !pEntity ) {
		...
		// Find the exit node
		m_hExitNode = gEntList.FindEntityByName( nullptr, m_iszExitNode );
		if ( !m_hExitNode ) {
...
// Input  : iCount -
//-----------------------------------------------------------------------------
void CAI_ActBusyQueueGoal::RecalculateQueueCount() {
...
// Purpose:
//-----------------------------------------------------------------------------
void CAI_ActBusyQueueGoal::MoveQueueUpThink() {
...
//-----------------------------------------------------------------------------
void CAI_ActBusyQueueGoal::QueueThink() {
	if ( !GetNPCOnNode( 0 ) ) {
		MoveQueueUp();
	}
