...
void CBreakable::DamageSound( void )
{
	...
	{
		pitch = 95 + RandomInt(0,34);
	}

	fvol = RandomFloat(0.75, 1.0);

	if (material == matComputer && RandomInt(0,1))
	{
		material = matMetal;
	}
...
void CBreakable::Die( void )
{
	...
	fvol = RandomFloat(0.85, 1.0) + (abs(m_iHealth) / 100.0);
	if (fvol > 1.0)
	{
		fvol = 1.0;
	}
