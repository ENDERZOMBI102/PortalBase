...
// Purpose:
//-----------------------------------------------------------------------------
void CTeamControlPointMaster::Spawn() {
	Precache();
	...
	if ( g_hControlPointMasters.Find( this ) == CUtlVector<CHandle<CTeamControlPointMaster>>::InvalidIndex() ) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CTeamControlPointMaster::UpdateOnRemove() {
...
// Purpose:
//-----------------------------------------------------------------------------
void CTeamControlPointMaster::Precache() {
...
// Purpose:
//-----------------------------------------------------------------------------
void CTeamControlPointMaster::Activate() {
	BaseClass::Activate();
...
// Purpose:
//-----------------------------------------------------------------------------
void CTeamControlPointMaster::RoundRespawn() { }

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void CTeamControlPointMaster::Reset() { }

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
bool CTeamControlPointMaster::FindControlPoints() {
	// go through all the points
	CBaseEntity* pEnt = gEntList.FindEntityByClassname( nullptr, GetControlPointName() );

	int numFound = 0;

	while ( pEnt ) {
		auto* pPoint = assert_cast<CTeamControlPoint*>( pEnt );
		...
		if ( pPoint->IsActive() && !pPoint->IsMarkedForDeletion() ) {
			...
			if ( m_ControlPoints.Find( index ) == CUtlMap<int, CTeamControlPoint*>::InvalidIndex() ) {
	...
	while ( newIndex < numPoints ) {
		...
		m_ControlPoints[ lowestIndex ]->SetPointIndex( static_cast<int>( newIndex ) );
		newIndex++;
	}
	...
	ObjectiveResource()->SetNumControlPoints( static_cast<int>( m_ControlPoints.Count() ) );
...
// Purpose:
//-----------------------------------------------------------------------------
void CTeamControlPointMaster::SetBaseControlPoints() {
	for ( int team = 0; team < GetNumberOfTeams(); team++ ) {
...
// Purpose:
//-----------------------------------------------------------------------------
bool CTeamControlPointMaster::FindControlPointRounds() {
	bool bFoundRounds = false;

	m_ControlPointRounds.RemoveAll();
	CBaseEntity* pEnt = gEntList.FindEntityByClassname( nullptr, GetControlPointRoundName() );

	while ( pEnt ) {
		auto* pRound = assert_cast<CTeamControlPointRound*>( pEnt );

		if ( pRound && ( m_ControlPointRounds.Find( pRound ) == CUtlVector<CTeamControlPoint*>::InvalidIndex() ) ) {
...
// Purpose:
//-----------------------------------------------------------------------------
int CTeamControlPointMaster::NumPlayableControlPointRounds() {
	int nRetVal = 0;
...
// Purpose:
//-----------------------------------------------------------------------------
bool CTeamControlPointMaster::SelectSpecificRound() {
	CTeamControlPointRound* pRound = nullptr;
	auto* pRules = dynamic_cast<CTeamplayRoundBasedRules*>( GameRules() );

	if ( pRules ) {
		if ( pRules->GetRoundToPlayNext() != NULL_STRING ) {
			// do we have the name of a round?
			pRound = dynamic_cast<CTeamControlPointRound*>( gEntList.FindEntityByName( nullptr, STRING( pRules->GetRoundToPlayNext() ) ) );

			if ( pRound ) {
				if ( ( m_ControlPointRounds.Find( pRound ) == CUtlVector<CTeamControlPoint*>::InvalidIndex() ) ||
...
// Purpose:
//-----------------------------------------------------------------------------
void CTeamControlPointMaster::RegisterRoundBeingPlayed() {
...
// Purpose:
//-----------------------------------------------------------------------------
bool CTeamControlPointMaster::GetControlPointRoundToPlay() {
	int i = 0;
	...
	if ( i >= m_ControlPointRounds.Count() || m_ControlPointRounds[ i ] == nullptr ) {
...
//-----------------------------------------------------------------------------
void CTeamControlPointMaster::CPMThink() {
...
// Purpose:
//-----------------------------------------------------------------------------
void CTeamControlPointMaster::CheckWinConditions() {
	if ( m_bDisabled )
		return;
		...
		if ( ( m_iInvalidCapWinner != 1 ) &&
			...
			bool bWinner = true;
				...
				if ( TFGameRules() && TFGameRules()->IsInKothMode() ) {
					CTeamRoundTimer* pTimer = nullptr;
...
// Purpose:
//-----------------------------------------------------------------------------
void CTeamControlPointMaster::HandleRandomOwnerControlPoints() {
...
void CTeamControlPointMaster::InputRoundActivate( inputdata_t& input ) {
	...
	if ( PlayingMiniRounds() && GetCurrentRound() == nullptr ) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CTeamControlPointMaster::FireRoundStartOutput() {
	CTeamControlPointRound* pRound = GetCurrentRound();
...
// Purpose:
//-----------------------------------------------------------------------------
void CTeamControlPointMaster::FireRoundEndOutput() {
	CTeamControlPointRound* pRound = GetCurrentRound();
...
//			win the game.
//-----------------------------------------------------------------------------
int CTeamControlPointMaster::TeamOwnsAllPoints( CTeamControlPoint* pOverridePoint /* = nullptr */, int iOverrideNewTeam /* = TEAM_UNASSIGNED */ ) {
	int iWinningTeam[ MAX_CONTROL_POINT_GROUPS ];

	for ( int i = 0; i < MAX_CONTROL_POINT_GROUPS; i++ ) {
...
int CTeamControlPointMaster::GetBaseControlPoint( int iTeam ) {
	...
	int nHighestValue = -1;
	CTeamControlPoint* pLowestPoint = nullptr;
	CTeamControlPoint* pHighestPoint = nullptr;
	...
	while ( vecControlPointsOwned.Count() < (int) m_ControlPoints.Count() ) {
		...
		for ( int i = 0; i < m_ControlPointRounds.Count(); ++i ) {
			...
			for ( int iControlPoint = 0; iControlPoint < pRound->m_ControlPoints.Count(); iControlPoint++ ) {
				...
				if ( CUtlVector<CTeamControlPoint*>::InvalidIndex() != vecControlPointsOwned.Find( pControlPoint ) ) {
...
// Purpose:
//-----------------------------------------------------------------------------
float CTeamControlPointMaster::GetPartialCapturePointRate() const {
	...
	// Purpose:
	//-----------------------------------------------------------------------------
	void CTeamControlPointMaster::ListRounds() {
		if ( PlayingMiniRounds() ) {
	...
	//-----------------------------------------------------------------------------
	void cc_ListRounds() {
		CTeamControlPointMaster* pMaster = g_hControlPointMasters.Count() ? g_hControlPointMasters[ 0 ] : nullptr;
		if ( pMaster ) {
			pMaster->ListRounds();
		}
	}
		...
		if ( args.ArgC() > 1 ) {
			...
			if ( pRules && pMaster ) {
				if ( pMaster->PlayingMiniRounds() ) {
					...
					auto* pRound = dynamic_cast<CTeamControlPointRound*>( gEntList.FindEntityByName( nullptr, args[ 1 ] ) );
