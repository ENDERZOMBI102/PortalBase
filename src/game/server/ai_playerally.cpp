...
public:
	CConceptInfoMap() :
	  CUtlMap<AIConcept_t, ConceptInfo_t *>( CaselessStringLessThan )
	  {
		  ...
		  {
			  Insert( g_ConceptInfos[i]._concept, &g_ConceptInfos[i] );
		  }
	  }
};
...
void CAI_AllySpeechManager::Spawn()
{
	...
	for ( int i = 0; i < ARRAYSIZE(g_ConceptInfos); i++ )
		m_ConceptTimers.Insert( AllocPooledString( g_ConceptInfos[i]._concept ), CSimpleSimTimer() );
}
...
void CAI_AllySpeechManager::AddCustomConcept( const ConceptInfo_t &conceptInfo )
{
	...
	if ( g_ConceptInfoMap.Find( conceptInfo._concept ) == g_ConceptInfoMap.InvalidIndex() )
	{
		g_ConceptInfoMap.Insert( conceptInfo._concept, new ConceptInfo_t( conceptInfo ) );
	}

	if ( m_ConceptTimers.Find( AllocPooledString(conceptInfo._concept) ) == m_ConceptTimers.InvalidIndex() )
	{
		m_ConceptTimers.Insert( AllocPooledString( conceptInfo._concept ), CSimpleSimTimer() );
	}
}
...
ConceptInfo_t *CAI_AllySpeechManager::GetConceptInfo( AIConcept_t _concept )
{
	int iResult = g_ConceptInfoMap.Find( _concept );

	return ( iResult != g_ConceptInfoMap.InvalidIndex() ) ? g_ConceptInfoMap[iResult] : NULL;
}

void CAI_AllySpeechManager::OnSpokeConcept( CAI_PlayerAlly *pPlayerAlly, AIConcept_t _concept, AI_Response *response )
{
	ConceptInfo_t *			pConceptInfo	= GetConceptInfo( _concept );
		...
		if ( pConceptInfo->flags & AICF_PROPAGATE_SPOKEN ) 
		{
			...
			for ( int i = 0; i < g_AI_Manager.NumAIs(); i++ )
			{
					 ...
					 pPlayerAlly->FVisible( pTalker ) )
				{
					...
					pTalker->GetExpresser()->SetSpokeConcept( _concept, NULL, false );
				}
			}
		}
	}
	...
	if ( !ai_no_talk_delay.GetBool() )
	{
		...
		if ( pConceptInfo && pConceptInfo->minConceptDelay != -1 )
		{
			...
			char iConceptTimer = m_ConceptTimers.Find( MAKE_STRING(_concept) );
...
bool CAI_AllySpeechManager::ConceptDelayExpired( AIConcept_t _concept )
{
	char iConceptTimer = m_ConceptTimers.Find( MAKE_STRING(_concept) );
	if ( iConceptTimer != m_ConceptTimers.InvalidIndex() )
		 ...
		 && !HasCondition(COND_RECEIVED_ORDERS) && !IsInAScript() )
	{
		if ( m_flNextIdleSpeechTime && m_flNextIdleSpeechTime < gpGlobals->curtime )
		{
			...
			if ( SelectNonCombatSpeech( &selection ) )
			{
				...
				SpeakDispatchResponse( selection._concept.c_str(), selection.Response );
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CAI_PlayerAlly::SelectSpeechResponse( AIConcept_t _concept, const char *pszModifiers, CBaseEntity *pTarget, AISpeechSelection_t *pSelection )
{
	if ( IsAllowedToSpeak( _concept ) )
	{
		bool result = SpeakFindResponse( pSelection->Response, _concept, pszModifiers );
		if ( result )
		{
			pSelection->_concept = _concept;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CAI_PlayerAlly::SetPendingSpeech( AIConcept_t _concept, AI_Response &Response )
{
	m_PendingResponse = Response;
	m_PendingConcept = _concept;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CAI_PlayerAlly::PostSpeakDispatchResponse( AIConcept_t _concept, AI_Response *response )
{
#ifdef HL2_EPISODIC
	CAI_AllySpeechManager *pSpeechManager = GetAllySpeechManager();
	ConceptInfo_t *pConceptInfo	= pSpeechManager->GetConceptInfo( _concept );
	if ( pConceptInfo && (pConceptInfo->flags & AICF_QUESTION) && GetSpeechTarget() )
	{
		bool bSaidHelloToNPC = !Q_strcmp(_concept, "TLK_HELLO_NPC");
		...
		if ( rr_debug_qa.GetBool() )
		{
			if ( bSaidHelloToNPC )
			{
				Warning("Q&A: '%s' said Hello to '%s' (_concept %s)\n", GetDebugName(), GetSpeechTarget()->GetDebugName(), _concept );
			}
			else
			{
				Warning("Q&A: '%s' questioned '%s' (_concept %s)\n", GetDebugName(), GetSpeechTarget()->GetDebugName(), _concept );
			}
		...
		{
			AddLookTarget( GetSpeechTarget()->MyNPCPointer(), 1.0, duration + RandomFloat( 0.4, 1.2 ), 0.5 );
			GetSpeechTarget()->MyNPCPointer()->AddLookTarget( this, 1.0, duration + RandomFloat( 0.4, 1 ), 0.7 );
		}

		// Don't let anyone else butt in.
		DeferAllIdleSpeech( RandomFloat( TALKER_DEFER_IDLE_SPEAK_MIN, TALKER_DEFER_IDLE_SPEAK_MAX ), GetSpeechTarget()->MyNPCPointer() );
	}
	else if ( pConceptInfo && (pConceptInfo->flags & AICF_ANSWER) && GetSpeechTarget() )
	{
		...
		if ( GetSpeechTarget()->MyNPCPointer() )
		{
			AddLookTarget( GetSpeechTarget()->MyNPCPointer(), 1.0, duration + RandomFloat( 0, 0.3 ), 0.5 );
			GetSpeechTarget()->MyNPCPointer()->AddLookTarget( this, 1.0, duration + RandomFloat( 0.2, 0.5 ), 0.7 );
		}
	}
...
int CAI_PlayerAlly::SelectNonCombatSpeechSchedule()
{
	if ( !HasPendingSpeech() )
	{
		...
		{
			SetSpeechTarget( selection.hSpeechTarget );
			SetPendingSpeech( selection._concept.c_str(), selection.Response );
		}
	}
...
//-----------------------------------------------------------------------------
bool CAI_PlayerAlly::ShouldSpeakRandom( AIConcept_t _concept, int iChance )
{
	CAI_AllySpeechManager *	pSpeechManager	= GetAllySpeechManager();
	ConceptInfo_t *			pInfo			= pSpeechManager->GetConceptInfo( _concept );
	...
	return (RandomInt(1,iChance) == 1);
}

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CAI_PlayerAlly::IsAllowedToSpeak( AIConcept_t _concept, bool bRespondingToPlayer )
{ 
	CAI_AllySpeechManager *	pSpeechManager	= GetAllySpeechManager();
	ConceptInfo_t *			pInfo			= pSpeechManager->GetConceptInfo( _concept );
	ConceptCategory_t		category		= ( pInfo ) ? pInfo->category : SPEECH_IDLE;
	...
	{
		if ( CompareConcepts( _concept, TLK_HELLO ) )
			return false;
		if ( CompareConcepts( _concept, TLK_HELLO_NPC ) )
			return false;
	}
			
	if ( !pSpeechManager->ConceptDelayExpired( _concept ) )
		return false;

	if ( ( pInfo && pInfo->flags & AICF_SPEAK_ONCE ) && GetExpresser()->SpokeConcept( _concept ) )
		return false;

	if ( !GetExpresser()->CanSpeakConcept( _concept ) )
		return false;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CAI_PlayerAlly::SpeakIfAllowed( AIConcept_t _concept, const char *modifiers, bool bRespondingToPlayer, char *pszOutResponseChosen, size_t bufsize )
{ 
	if ( IsAllowedToSpeak( _concept, bRespondingToPlayer ) )
	{
		return Speak( _concept, modifiers, pszOutResponseChosen, bufsize );
	}
	return false;
}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CAI_PlayerAlly::OnSpokeConcept( AIConcept_t _concept, AI_Response *response )
{
	CAI_AllySpeechManager *pSpeechManager = GetAllySpeechManager();
	pSpeechManager->OnSpokeConcept( this, _concept, response );
